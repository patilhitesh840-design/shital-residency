...
async function renderMembers(container) {
    ...
    container.innerHTML = `
        <div class="space-y-6 animate-fade-in">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    <select id="member-filter-building" class="w-full px-4 py-2.5 rounded-xl glass-input text-xs text-slate-800 dark:text-slate-100">
    ...
    const updateTable = () => {
        ...
        const filterBld = container.querySelector('#member-filter-building').value;
    ...
    container.querySelector('#member-filter-building').addEventListener('change', updateTable);
...
function showMemberModal(member = null) {
            ...
            <div class="w-full max-w-md glass-panel border border-glass-border-light dark:border-glass-border-dark rounded-2xl shadow-2xl p-6 animate-scale-up">
                ...
                <form id="member-modal-form" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1">
                            <label class="text-[10px] uppercase tracking-wider font-semibold text-slate-400">Building Block</label>
                            <select id="modal-building" required class="w-full px-3 py-2 rounded-xl glass-input text-xs text-slate-800 dark:text-slate-100">
                                <option value="A" ${isEdit && member.building === 'A' ? 'selected' : ''}>A Wing</option>
                                <option value="B" ${isEdit && member.building === 'B' ? 'selected' : ''}>B Wing</option>
                                <option value="C" ${isEdit && member.building === 'C' ? 'selected' : ''}>C Wing</option>
                            </select>
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] uppercase tracking-wider font-semibold text-slate-400">Flat Number</label>
    ...
    modalContainer.querySelector('#member-modal-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const data = {
            building: document.getElementById('modal-building').value,
